"""Pure audio transcoding between VoiceAI Connect coders and the Gemini Live rates.

VoiceAI Connect negotiates a media format from `supportedMediaFormats` (mu-law or
linear PCM16 at 8 / 16 / 24 kHz). The Live Translate model takes **16 kHz PCM16 in** and
...
MODEL_IN_RATE = 16000   # what Live Translate accepts (audio/pcm;rate=16000)
MODEL_OUT_RATE = 24000  # what Live Translate emits
...
# Caller-in preference: hit the model's 16 kHz input with no resampling if we can;
...
# Play-out preference: ship the model's native 24 kHz untouched if offered;
...
def select_formats(supported: list[str]) -> tuple[MediaFormat, MediaFormat]:
    ...
    `playStream.start` format, each the closest match to the model's native
...
def decode_to_pcm16(data: bytes, fmt: MediaFormat, target_rate: int) -> bytes:
    """A VAIC userStream chunk in `fmt` -> PCM16 at `target_rate` (model input)."""
    if fmt.wav:
...
def encode_from_pcm16(pcm: bytes, src_rate: int, fmt: MediaFormat) -> bytes:
    """Model PCM16 at `src_rate` -> a VAIC playStream chunk in `fmt`."""
