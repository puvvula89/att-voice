...
VoiceAI Connect negotiates a coder (mu-law / linear PCM at 8/16/24 kHz); the Live
model takes 16 kHz PCM16 in and 24 kHz PCM16 out. These pure functions bridge
...
def test_select_prefers_native_rates():
    ...
    # both model-native so no resampling.
    supported = [
        "raw/mulaw", "raw/lpcm16", "raw/lpcm16_8", "raw/lpcm16_24",
    ]
    user_fmt, play_fmt = select_formats(supported)
    assert user_fmt.name == "raw/lpcm16"      # 16k in = model input rate
    assert play_fmt.name == "raw/lpcm16_24"   # 24k out = model output rate
...
# --------------------------------------------------------------------------- #
# Format decode/encode round-trips through the model rates                    #
# --------------------------------------------------------------------------- #
def test_decode_lpcm16_native_is_passthrough_to_model_rate():
    pcm16 = _pcm([0, 500, -500, 1000])
    fmt = parse_format("raw/lpcm16")  # already 16k
    out = decode_to_pcm16(pcm16, fmt, MODEL_IN_RATE)
    assert out == pcm16
...
def test_decode_8k_upsamples_to_16k():
    ...
    out = decode_to_pcm16(pcm8, fmt, MODEL_IN_RATE)
    assert len(_samples(out)) == 160  # upsampled to 16k


def test_encode_24k_model_audio_to_8k_mulaw():
    pcm24 = _pcm([2000] * 240)  # 10 ms @ 24k
    fmt = parse_format("raw/mulaw")
    out = encode_from_pcm16(pcm24, MODEL_OUT_RATE, fmt)
