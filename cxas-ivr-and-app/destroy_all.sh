...
#
#   default   1  UI            Cloud Run: ivr-and-app-ui        (public)
#             2  relay         Cloud Run: ivr-and-app-relay     (public)
#             3  hydration     Cloud Run: ivr-and-app-hydration     (private)
...
echo "  Cloud Run : ${UI_SERVICE:-ivr-and-app-ui}, ${RELAY_SERVICE:-ivr-and-app-relay}, ${HYDRATION_SERVICE:-ivr-and-app-hydration}"
if $DELETE_APPS; then
  echo "  CXAS app  : $VOICE_APP   ← INCLUDING ALL CONVERSATION HISTORY"
else
...
for service in "${UI_SERVICE:-ivr-and-app-ui}" \
               "${RELAY_SERVICE:-ivr-and-app-relay}" \
               "${HYDRATION_SERVICE:-ivr-and-app-hydration}"; do
...
# --- 4-5. Toolset, variables, then the app itself -----------------------
echo
echo "▶ deleting CXAS toolset, variables, and app"
"$PY" - "$VOICE_APP" <<'PYEOF'
import os
import sys
