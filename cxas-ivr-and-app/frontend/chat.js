...
generateBtn.onclick = () => setSessionId(uuid());

// The session id is the handoff token between channels: copy it here, paste it on
// the IVR side to continue the same conversation.
copyBtn.onclick = async () => {
  ...
  try {
    ...
    // Clipboard API needs a secure context; select the text so Cmd-C still works.
    userInput.focus();
    userInput.select();
    return;
  }
...
/** Lock identity while a session is live — changing it mid-call would be a no-op
 *  on the open stream and silently mislead about which conversation is running. */
function lockIdentity(locked) {
  userInput.readOnly = locked;
...
function endChat() {
  ...
  teardown();
  // teardown() nulls ws.onclose, so the unlock that normally rides on it never
  // fires when the AGENT ends the session — do it here or the id stays frozen.
  lockIdentity(false);
