...
VOICE_APP="${VOICE_APP_ID:-ivr-and-app-voice}"
HYDRATION_SERVICE="${HYDRATION_SERVICE:-ivr-and-app-hydration}"
RELAY_SERVICE="${RELAY_SERVICE:-ivr-and-app-relay}"
UI_SERVICE="${UI_SERVICE:-ivr-and-app-ui}"
...
# --- 1. voice app ------------------------------------------------------------
echo
echo "▶ [1/5] CX Agent Studio app: $VOICE_APP"
...
# --- 2. hydration service (private) + run.invoker + toolset -------------------
...
echo
echo "▶ [2/5] Hydration service → Cloud Run (private, warm, boosted)"
...
# --- 3. attach the toolset and the firing callback ---------------------------
echo
echo "▶ [3/5] Attach hydration toolset + before_model callback to the root agent"
...
echo
echo "▶ [4/5] Relay → Cloud Run (public, warm, long-lived sockets)"
